...
from tnb.accounts.key_files import (
...
def test_create_account_and_save_signing_key_file(tmpdir):
    ...
    signing_key, account_number = create_account_and_save_signing_key_file(
        file_path)
...
def test_read_signing_key_file(tmpdir):
    file_path = tmpdir.join("signing_key_file")
    signing_key, account_number = \
        create_account_and_save_signing_key_file(file_path)
